...
const COL = {
  ...
  FABRIC_RATE:7, TOTAL_FABRIC_AMOUNT:8,
  CUTTING_MASTER:9, STYLE:10,
  FAB_LENGTH:11, FAB_WIDTH:12, GSM:13, NUM_PIECES:14,
  CLOTH_USED:15, FABRIC_COST_PIECE:16, TOTAL_FABRIC_COST:17,
  JOB_DESC:18, JOB_COST:19, FIXED_COST:20, ACCESSORIES_COST:21, MRP_PCT:22,
  VARIABLE_COST:23, FINAL_MRP:24, NOTES:25
};
...
function recalc() {
  ...
  const clothUsed = numPieces > 0 ? totalFabricAmount / numPieces : 0;
  ...
  const finalMRP = (variableCostPerPiece + fixedCost + accessoriesCost) * (1 + mrpPercent / 100);

  document.getElementById("calcClothUsed").textContent = clothUsed.toFixed(3);
  ...
  document.getElementById("calcAccessories").textContent = "₹" + accessoriesCost.toFixed(2);
  document.getElementById("calcMRP").textContent = "₹" + finalMRP.toFixed(2);

  return { clothUsed, fabricCostPerPiece, totalFabricCost, variableCostPerPiece, accessoriesCost, finalMRP };
}
...
function viewDetail(rowIndex) {
  ...
  const html = `
    ...
    <div class="detail-grid">
      ...
      <div class="detail-item"><div class="k">Cloth Used / Piece</div><div class="v">${get(COL.CLOTH_USED)} kg/m</div></div>
